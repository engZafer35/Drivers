...
* #Revision     : 1.0
* #Date         : Oct 9, 2019 - 1:53:24 PM
* #File Name    : JsonParser.c 
* #File Path    : /MyJson/src/JsonParser.c
*******************************************************************************/
/******************************************************************************
*
*
...
int jsonGetData(const char *json, const char *key, jsmnData *data)
{
    ...
    jsmn_init(&p);
    r = jsmnParse(&p, json, strlen(json), t, sizeof(t) / sizeof(t[0]));
...
//      return 1;
    ...
    for (i = 1; i < r; i++)
    {
      if (jsoneq(json, &t[i], key) == 0) {
        ...
        sprintf(data->str, "%.*s", t[i + 1].end - t[i + 1].start, json + t[i + 1].start);
        i++;
        retVal = 0;
        break;
      }
